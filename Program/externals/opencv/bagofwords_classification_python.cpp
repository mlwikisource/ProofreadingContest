...
void help(char** argv)
{
    ...
    << "[result directory]     Path to result directory. Following folders will be created in [result directory]: \n"
...
vector<float> computeConfidences( CvSVM& svm, const string& objClassName, VocData& vocData,
/*void computeConfidences( CvSVM& svm, const string& objClassName, VocData& vocData,*/
                         Ptr<BOWImgDescriptorExtractor>& bowExtractor, const Ptr<FeatureDetector>& fdetector,
                         const string& resPath, const vector<string>& files )
{
    ...
    if ( files.size() )
    {
        // copied from from 'VocData::convertImageCodesToObdImages' (hacky)
        //cout << images[0].id << " " << images[0].path << endl;
        // 000005 /data/toolserver/pywikipedia/opencv/VOC2007//JPEGImages/000005.jpg
        images.clear();
        ...
        for (size_t i = 0; i < files.size(); ++i)
        {
            //generate image path and indices from extracted string code
            path = files[i];
            images.push_back(ObdImage(boost::str(boost::format("%06i")%i), path));
            //cout << images[i].id << " " << images[i].path << endl;
        }
        //images.clear();
    }
                     ...
                     const boost::python::list& files_list = boost::python::list())
{
    ...
    for( size_t classIdx = 0; classIdx < objClasses.size(); ++classIdx )
    {
        ...
        /*computeConfidences( svm, objClasses[classIdx], vocData,
                            bowExtractor, featureDetector, resPath );*/
        conf = computeConfidences( svm, objClasses[classIdx], vocData,
                            bowExtractor, featureDetector, resPath, files_vec );
        std::copy( conf.begin(), conf.end(), back_inserter(confidences) );
...
int main(int argc, char** argv)
{
    ...
    {
        Py_Initialize();
        confidences = worker(argc, argv[1], argv[2]);
    }
    else if ( argc == 6 )
    {
        Py_Initialize();
        confidences = worker(argc, argv[1], argv[2], argv[3], argv[4], argv[5]);
    }
    else
    {
    	help(argv);
...
BOOST_PYTHON_MODULE(BoWclassify)
{
  ...
  //;
  def("main", worker, "BoWclassify main function");
}
